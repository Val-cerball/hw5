...
//             conditions and forecast based on the number of days entered by the user.
 ...
 window.addEventListener('DOMContentLoaded', async function() {
    ...
    weatherButton.addEventListener (`click`, async function (event){
      ...
      // - Get a reference to the days element
     ...
     if (days.length >0) {
      ...
      // - Store the interpreted forecast days as a variable
      let daysResult = json.forecast.forecastday
  
      // - Fill the forecast days entered in the title 
      ...
      `
       // loop through the forecast details
      for (let i=0; i<daysResult.length; i++) {
  
        // Create a variable to store each forecast detail in memory
